...
export const buildDeck = (
  ...
  switch (mode) {
    ...
    case 'words-only':
      return shuffleArray(
        getAllWords(language).filter((word) => word.kind === 'word'),
      );
    case 'phrases-only':
      return shuffleArray(
        getAllWords(language).filter((word) => word.kind === 'phrase'),
      );
    case 'weak-words':
...
export const getBackLines = (
  ...
  if (mode === 'ru-to-foreign' || mode === 'weak-words' || mode === 'favorites' || mode === 'single-topic' || mode === 'words-only' || mode === 'phrases-only') {
    return word.terms;
  }
